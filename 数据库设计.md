...
- ### 字段详情
 ...
 | role     | character varying(20)  | CHECK (role IN ('teacher','admin')) | 'teacher' | 用户角色（teacher, admin）|
...
| teacher_id  | integer           | NOT NULL, FOREIGN KEY(users.id) ON DELETE RESTRICT, INDEX | - | 教师ID |
| status      | character varying(50)  | CHECK (status IN ('active','inactive','draft')) | 'active' | 课程状态（active, inactive, draft） |
  | created_at  | datetime(timezone=True) | -                     | now()                             | 创建时间             |
  | updated_at  | datetime(timezone=True) | -                     | now() on update                   | 更新时间             |
...
- ### 索引信息
 ...
 | ix_courses_teacher_created | btree | teacher_id, created_at | 教师+创建时间复合索引 |
 | uq_courses_teacher_title | unique | teacher_id, title | 同一教师下课程标题唯一 |
...
| course_id   | integer           | NOT NULL, FOREIGN KEY(courses.id) ON DELETE CASCADE, INDEX | - | 课程ID |
